        ...
        for key, value in condition.items():
            if key == 'Subject':
                ...
                for num_key, num_values in value.items():
                    ...
                    elif num_key == 'NOTBETWEEN':
                        num_condition = num_values[0]
                        clauses.append(f"{subject} NOT BETWEEN {num_condition}")
...
def build_sql_query(dic_main, dic_if):
    ...
    sql_query = f"SELECT * FROM {table_name} WHERE  {final_where_clause.replace(' AND ', ' AND ').replace(' OR ', ' OR ')};"
    return sql_query
