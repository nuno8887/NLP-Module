...
split_patterns = [
    ...
    {"label": "MODAL_OPERATOR_OBLIGATION", "pattern": [{"LOWER": "must"}]},# must
...
def identify_modal_operators(doc):
    ...
    for ent in doc.ents:
        if ent.label_ in {"MODAL_OPERATOR_PERMISSION", "MODAL_OPERATOR_OBLIGATION", "MODAL_OPERATOR_PROHIBITION"}:
            modal_label = ent.label_
            break  # Stop after finding the first relevant label
